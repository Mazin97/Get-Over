...
export default function Triagem({ navigation }) {
  ...
  function verificarIndexPacient() {
  ...
  function loadPerguntas() {
    api.get('/pergunta', { headers: { isTriagemPaciente: true } }).then((response) => {
  ...
  function EnviarPerguntas() {
    ...
    api.put('/usuario', {
      id: id,
      indexPacient: mediaGeral
    }).then((res) => {
      AsyncStorage.setItem('indexPacient', res.data.indexPacient.toString()).then(() => {
        navigation.navigate('Novo Post');
      });
    }).catch((error) => {
      console.log(error);
